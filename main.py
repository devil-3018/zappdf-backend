"""
ZapPDF by reNexaris — Backend API
Clean version with CORS fully fixed
"""
...
# ── APP ───────────────────────────────────────────────
app = FastAPI(title="ZapPDF by reNexaris", version="3.0.0")
...
class CORSFix(BaseHTTPMiddleware):
...
def pres(p, n):
    return FileResponse(
        str(p), media_type="application/pdf", filename=n,
        headers={"Access-Control-Allow-Origin": "*"}
    )
def fmtSz(b): return f"{b/1024/1024:.1f}MB"

# ── HEALTH CHECK ──────────────────────────────────────
@app.get("/")
def root():
    return {"status": "ZapPDF by reNexaris", "version": "3.0.0", "endpoints": 27}
...
# ── 2. SPLIT ──────────────────────────────────────────
@app.post("/split")
async def split(file: UploadFile = File(...), mode: str = Form("each"),
                ranges: str = Form(""), fixed: int = Form(1)):
...
async def compress(file: UploadFile = File(...), level: str = Form("medium")):
    ...
    return pres(out, "compressed.pdf")
...
async def rotate(file: UploadFile = File(...), degrees: int = Form(90),
    ...
    for i, pg in enumerate(r.pages):
        if pages == "all" or (pages == "even" and (i+1)%2==0) or \
           (pages == "odd" and (i+1)%2==1) or str(i+1) in pages.split(","):
            pg.rotate(degrees)
        w.add_page(pg)
    out = tmp("rotated.pdf")
    with open(out, "wb") as f: w.write(f)
...
async def watermark(file: UploadFile = File(...), text: str = Form("CONFIDENTIAL"),
                    opacity: float = Form(0.3), position: str = Form("center"),
    ...
    for page in doc:
        ...
        page.insert_text(fitz.Point(w*pos[0], h*pos[1]), text,
                        fontsize=font_size, color=(0.5,0.5,0.5), overlay=True)
...
async def page_numbers(file: UploadFile = File(...), position: str = Form("bottom-center"),
    ...
    for i, page in enumerate(doc):
        ...
        pm = {"bottom-center": fitz.Point(w/2-8, h-18), "bottom-right": fitz.Point(w-36, h-18),
              "bottom-left": fitz.Point(18, h-18), "top-center": fitz.Point(w/2-8, 20),
              "top-right": fitz.Point(w-36, 20)}
        page.insert_text(pm.get(position, fitz.Point(w/2-8, h-18)), str(n),
...
# ── 11. CROP ──────────────────────────────────────────
@app.post("/crop")
async def crop(file: UploadFile = File(...), left: float = Form(20),
               top: float = Form(20), right: float = Form(20), bottom: float = Form(20)):
...
async def compare(files: List[UploadFile] = File(...)):
    ...
    for i in range(max(len(d1), len(d2))):
        ...
        results.append({"page": i+1, "identical": t1.strip()==t2.strip()})
    changed = sum(1 for r in results if not r["identical"])
    return JSONResponse({"total_pages": len(results), "changed_pages": changed,
                         "identical_pages": len(results)-changed, "pages": results})
...
async def jpg_to_pdf(files: List[UploadFile] = File(...),
                     orientation: str = Form("auto"), margin_mm: int = Form(10)):
    doc = fitz.open(); margin = margin_mm * 2.835
    for i, f in enumerate(files):
        p = save(f, f"img{i}.tmp")
        try:
            pix = fitz.Pixmap(str(p)); w_px, h_px = pix.width, pix.height
        except: continue
        ...
        page.insert_image(fitz.Rect(margin, margin, page.rect.width-margin, page.rect.height-margin),
                         filename=str(p))
...
async def excel_to_pdf(file: UploadFile = File(...)):
    try: import openpyxl
    except: raise HTTPException(500, "openpyxl not installed")
...
async def ppt_to_pdf(file: UploadFile = File(...)):
    ...
    except: raise HTTPException(500, "python-pptx not installed")
    ...
    try:
        ...
        ST = ParagraphStyle('ST', parent=styles['Heading1'],
        ...
        SimpleDocTemplate(str(out), pagesize=A4,
            ...
            elements or [Paragraph("Empty", styles['Normal'])])
...
async def pdf_to_excel(file: UploadFile = File(...)):
    ...
    for i, page in enumerate(doc):
        ...
        ws['A1'] = f"Page {i+1}"
        ws['A1'].font = xl.styles.Font(bold=True, size=12)
...
async def ai_summarize(file: UploadFile = File(...), length: str = Form("standard"),
    ...
    if not key:
        return JSONResponse({"summary": "⚠️ Add ANTHROPIC_API_KEY to Railway environment variables to enable AI features.\n\nGo to: Railway → your project → Variables → Add ANTHROPIC_API_KEY", "pages": len(doc)})
    lm = {"brief": "3-5 bullet points", "standard": "2-3 paragraphs", "detailed": "comprehensive analysis"}
    fm = {"key_points": "key points", "action_items": "action items", "executive": "executive summary"}
...
# ── 25. AI CHAT ───────────────────────────────────────
@app.post("/ai/chat")
async def ai_chat(file: Optional[UploadFile] = File(None),
                  question: str = Form(...), context: str = Form(""),
                  api_key: str = Form("")):
    import httpx
    key = api_key or os.getenv("ANTHROPIC_API_KEY", "")
    doc_text = context
    ...
    if not key:
        return JSONResponse({"answer": "⚠️ Add ANTHROPIC_API_KEY to Railway environment variables to enable AI Chat."})
...
async def ai_translate(file: UploadFile = File(...),
                       target_language: str = Form("Spanish"), api_key: str = Form("")):
    import httpx
    key = api_key or os.getenv("ANTHROPIC_API_KEY", "")
    ...
    if not key:
        ...
        return pres(out, "translated.pdf")
    for i, page in enumerate(doc):
        text = page.get_text()
...
async def smart_redact(file: UploadFile = File(...),
                       detect: str = Form("names,emails,phones,ssn,credit_cards"),
                       api_key: str = Form("")):
    import httpx
    key = api_key or os.getenv("ANTHROPIC_API_KEY", "")
    ...
    resp = pres(out, "smart_redacted.pdf")
    resp.headers["X-Redacted"] = str(total)
    return resp

# ── PDF INFO ──────────────────────────────────────────
@app.post("/pdf-info")
async def pdf_info(file: UploadFile = File(...)):
    p = save(file, "info.pdf"); doc = fitz.open(str(p))
    meta = doc.metadata
    return JSONResponse({
        "pages": len(doc), "title": meta.get("title",""),
        "author": meta.get("author",""), "encrypted": doc.is_encrypted,
...
async def startup():
    print("✅ ZapPDF by reNexaris — API Started")
